...
function Scala.parse_cents(value)
...
function Scala.parse_ratio(value)
	-- get numerator
			...
			if desc == nil then
				...
				if expected_length == 0 then
					...
					if string.find(value, '%.') ~= nil then
						ratio = Scala.parse_cents(value)
					else
						ratio = Scala.parse_ratio(value)
					end
					ratios[length] = ratio
				end
			end
